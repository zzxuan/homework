...
class Uploader
{
    ...
    {
        $this->write_log("sdskdjaskldjlas");
        ...
        //$this->write_log("srcfile = ".$this->filePath);
        $info = pathinfo($this->filePath);
        $tempfile = dirname($this->filePath) . "temimg." . $info['extension'];
        //$this->write_log("tempfile = ".$tempfile);
        if (!(move_uploaded_file($file["tmp_name"], $tempfile) && file_exists($tempfile))) { //移动失败
            $this->stateInfo = $this->getStateInfo("ERROR_FILE_MOVE");
        } else { //移动成功
            //----压缩图片------
            if ($this->scal_pic($tempfile, $this->filePath)) {
                unlink($tempfile); //删除临时图片
            } else {
                rename($tempfile, $this->filePath); //重命名
            }
    ...
    {
        $handle = fopen("./uelog.txt", "a+");
        $text = date("Y-m-d H:i:s") . " " . $msg . "\r\n";
        fwrite($handle, $text);
        fclose($handle);
    }
    
        public function scal_pic($file_name, $file_new, $max_width = 650)
    {
        //验证参数
        ...
        //复制图象
        if (!@imagecopyresampled($new_pic, $pic_creat, 0, 0, 0, 0, $re_width, $re_height,
            $pic_scal_arr[0], $pic_scal_arr[1])) {
            return false;
        }
        //输出文件
        $out_file = '';
